...
async function signIn(page) {
    ...
    await page.getByRole('button', { name: 'Login with SSO' }).click({ noWaitAfter: true });
...
async function setPasswordLoginMode(page, mode) {
    await page.evaluate(async passwordLoginMode => {
        const config = await ApiClient.getPluginConfiguration('4c5b9b96-80cd-4c3d-9e3d-23fa4ebf6ce4');
        config.PasswordLoginMode = passwordLoginMode;
        await ApiClient.updatePluginConfiguration('4c5b9b96-80cd-4c3d-9e3d-23fa4ebf6ce4', config);
    }, mode);
}
...
async function providerAdmin(request) {
    const response = await request.post('https://oidc.localhost:8443/keycloak/realms/master/protocol/openid-connect/token', {
        form: {
            client_id: 'admin-cli',
            grant_type: 'password',
            username: 'admin',
...
async function providerUser(request, headers) {
    const response = await request.get('https://oidc.localhost:8443/keycloak/admin/realms/jellyfin/users', {
        headers,
...
async function providerGroup(request, headers, name) {
    const response = await request.get('https://oidc.localhost:8443/keycloak/admin/realms/jellyfin/groups', {
        headers,
...
async function authenticatePassword(request, username, password) {
    return request.post('https://localhost:8443/Users/AuthenticateByName', {
        headers: { Authorization: 'MediaBrowser Client="e2e", Device="e2e", DeviceId="e2e", Version="1"' },
...
test('provisions the allowed OIDC test user and starts a Jellyfin session', async ({ page }) => {
    await signIn(page);
...
test('shows an OIDC sign-in error on the login page', async ({ page }) => {
    ...
    await expect(page.locator('.toast')).toHaveText("We couldn't sign you in. Please try again.");
});

test('loads the seeded OIDC settings and validates changes for an administrator', async ({ page }) => {
    await signIn(page);
    await page.goto('/web/index.html#/configurationpage?name=OIDC%20Authentication');
    await expect(page.locator('#Enabled')).toBeChecked();
    await expect(page.locator('#PublicUrl')).toHaveValue('https://localhost:8443');
    await expect(page.locator('#IssuerUrl')).toHaveValue('https://oidc.localhost:8443/keycloak/realms/jellyfin');
    await expect(page.locator('#ClientId')).toHaveValue('jellyfin');
    await expect(page.locator('#UserGroup')).toHaveValue('jellyfin-users');
    await expect(page.locator('#AdministratorGroup')).toHaveValue('jellyfin-admins');
    ...
    expect(await page.locator('#IssuerUrl').evaluate(input => input.validity.valueMissing)).toBe(true);

    await page.locator('#IssuerUrl').fill('https://oidc.localhost:8443/keycloak/realms/jellyfin');
    await expect(page.locator('#SaveButton')).toBeDisabled();

    await page.locator('#PasswordLoginMode').selectOption('DisableForLinkedUsersOnly');
    await page.locator('#SaveButton').click();
    await expect(page.getByRole('heading', { name: 'Confirm password login change' })).toBeVisible();
    await expect(page.getByText('test OIDC sign-in in a separate browser session before signing out.')).toBeVisible();
    await page.getByRole('button', { name: 'Cancel' }).click();
});

test('only skips the Jellyfin login form when local passwords are disabled for all users', async ({ browser, page }) => {
    const adminPage = await browser.newPage();
    await signIn(adminPage);
    await adminPage.goto('/web/index.html#/configurationpage?name=OIDC%20Authentication');
    ...
    await showLogin(page);
    await expect(page).toHaveURL(/oidc\.localhost:8443\/keycloak\/realms\/jellyfin/, { timeout: 30_000 });
    await expect(page.getByRole('button', { name: 'Login with SSO' })).toHaveCount(0);
    await setPasswordLoginMode(adminPage, 'AllowForAllUsers');
    await adminPage.close();
});

test('synchronizes a Jellyfin administrator role after the Identity Provider group changes', async ({ browser, request }) => {
    const headers = await providerAdmin(request);
    const user = await providerUser(request, headers);
    const administratorGroup = await providerGroup(request, headers, 'jellyfin-admins');
    ...
    try {
        ...
        const updatedContext = await browser.newContext({ ignoreHTTPSErrors: true });
...
test('keeps an Identity Link when the Identity Provider email changes', async ({ browser, page, request }) => {
    await signIn(page);
    ...
    try {
        ...
        const update = await request.put(`https://oidc.localhost:8443/keycloak/admin/realms/jellyfin/users/${user.id}`, { headers, data: user });
        ...
        const restore = await request.put(`https://oidc.localhost:8443/keycloak/admin/realms/jellyfin/users/${user.id}`, { headers, data: user });
        expect(restore.ok()).toBe(true);
    }
});

test('enforces local password policy through Jellyfin authentication', async ({ browser, request }) => {
    const adminPage = await browser.newPage();
    await signIn(adminPage);
    await setPasswordLoginMode(adminPage, 'DisableForAllUsers');
...
test('disables local passwords only for linked Jellyfin Users', async ({ browser, request }) => {
    const adminPage = await browser.newPage();
    await signIn(adminPage);
    ...
    const setPassword = await request.post('https://localhost:8443/Users/Password', {
    ...
    expect((await authenticatePassword(request, user.Name, password)).ok()).toBe(true);
    ...
    try {
        expect((await authenticatePassword(request, user.Name, password)).ok()).toBe(false);
...
test('denies OIDC sign-in when the Identity Provider removes all allowed groups', async ({ browser, request }) => {
    const headers = await providerAdmin(request);
    const user = await providerUser(request, headers);
    ...
    try {
        for (const group of groups) {
            const remove = await request.delete(`https://oidc.localhost:8443/keycloak/admin/realms/jellyfin/users/${user.id}/groups/${group.id}`, { headers });
        ...
        for (const group of groups) {
            const restore = await request.put(`https://oidc.localhost:8443/keycloak/admin/realms/jellyfin/users/${user.id}/groups/${group.id}`, { headers });
            expect(restore.ok()).toBe(true);
        }
    }
});

test('denies a verified email that belongs to another Jellyfin User', async ({ browser, request }) => {
    const rootLogin = await authenticatePassword(request, 'root', '');
    expect(rootLogin.ok()).toBe(true);
    const { AccessToken } = await rootLogin.json();
    ...
    try {
        user.email = email;
        expect((await request.put(`https://oidc.localhost:8443/keycloak/admin/realms/jellyfin/users/${user.id}`, { headers, data: user })).ok()).toBe(true);
        const deniedContext = await browser.newContext({ ignoreHTTPSErrors: true });
        await signInDenied(await deniedContext.newPage());
        await deniedContext.close();
    } finally {
        user.email = originalEmail;
        expect((await request.put(`https://oidc.localhost:8443/keycloak/admin/realms/jellyfin/users/${user.id}`, { headers, data: user })).ok()).toBe(true);
        expect((await request.delete(`https://localhost:8443/Users/${localUser.Id}`, {
            headers: { Authorization: `MediaBrowser Token="${AccessToken}"` },
        })).ok()).toBe(true);
    }
});

test('reprovisions a Jellyfin User after its linked user is deleted', async ({ browser, page, request }) => {
    await signIn(page);
    const user = await currentUser(page);
    const rootLogin = await authenticatePassword(request, 'root', '');
    expect(rootLogin.ok()).toBe(true);
    const { AccessToken } = await rootLogin.json();
    expect((await request.delete(`https://localhost:8443/Users/${user.Id}`, {
        headers: { Authorization: `MediaBrowser Token="${AccessToken}"` },
    })).ok()).toBe(true);

    const returningContext = await browser.newContext({ ignoreHTTPSErrors: true });
...
test('returns to Jellyfin login when no logout token is available', async ({ page }) => {
    await page.goto('/oidc/logout');
    await expect(page).toHaveURL(/\/web\/index\.html#!\/login/);
});

test('requests Identity Provider logout when RP-initiated logout is enabled', async ({ browser }) => {
    const adminPage = await browser.newPage();
    await signIn(adminPage);
    ...
    try {
        ...
        await logoutPage.waitForFunction(() => window.oidcRpInitiatedLogout === true);
        const logoutRequest = logoutPage.waitForRequest(request => request.url().includes('/protocol/openid-connect/logout'));
        ...
        expect(url.searchParams.get('post_logout_redirect_uri')).toBe('https://localhost:8443/web/index.html#!/login');
